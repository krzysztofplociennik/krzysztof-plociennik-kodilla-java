...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        double days = project.getTaskLists().stream()
                ...
                .map(t -> LocalDate.now().getDayOfMonth()-t)
                .reduce(0, (sum, current) -> sum = sum + current);

        double instances = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(t -> t.getTasks().stream())
                .count();
    ...
    public void testAddTaskListAverageWorkingOnTaskWithAverage() {
        ...
        long max = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(t -> t.getTasks().stream())
                .count();

        int maxAsInt = (int)max;
        ...
        OptionalDouble result = IntStream.range(0, maxAsInt)
                .map(n -> tasks.get(n))
                .average();
